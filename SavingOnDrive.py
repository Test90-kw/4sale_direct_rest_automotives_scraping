...
class SavingOnDrive:
    ...
    def get_folder_id(self, folder_name, parent_folder_id):
        """Get folder ID by name within the specified parent folder."""
        try:
    ...
    def save_files(self, files):
        """Save files to Google Drive in both parent folders."""
        try:
            yesterday = (datetime.now() - timedelta(days=1)).strftime('%Y-%m-%d')

            for parent_folder_id in self.parent_folder_ids:
                folder_id = self.get_folder_id(yesterday, parent_folder_id)
                if not folder_id:
            ...
            print(f"Files uploaded successfully to both parent folders.")
        except Exception as e:
